...
#Supplementary: showing how the H, M, and FA and CN changes as with precipitation intensities

# function for categerical mterices ---------------------------------------


cat_met2 <- function(dt, thld){ 
  ...
  cat_met <- cat_met[, .(variable, POD, FAR, CSI, H, M, FA, CN)]
...
ind_met <- lapply(thresholds_ind, cat_met2, dt=ind_dat)
ind_met_dt <- rbindlist(ind_met)
ind_met_dt[, `:=`(thresh = rep(intensity, each = 3), ocn = factor('Indian'))]
ind_met_dt[, `:=`(thresh = rep(intensity, each = 3), ocn = factor('Indian'))]
...
atln_met <- lapply(thresholds_atln, cat_met2, dt=atln_dat)
atln_met_dt <- rbindlist(atln_met)
atln_met_dt[, `:=`(thresh = rep(intensity, each = 3), ocn = factor('Atlantic'))]
...
east_pacf_met <- lapply(thresholds_east_pacf, cat_met2, dt=east_pacf_dat)
east_pacf_met_dt <- rbindlist(east_pacf_met)
east_pacf_met_dt[, `:=`(thresh = rep(intensity, each = 3), ocn = factor('East Pacific'))]
...
west_pacf_met <- lapply(thresholds_west_pacf, cat_met2, dt=west_pacf_dat)
west_pacf_met_dt <- rbindlist(west_pacf_met)
west_pacf_met_dt[, `:=`(thresh = rep(intensity, each = 3), ocn = factor('West Pacific'))]
...
imrg_f <- ind_alt_pacf[variable == "IMERG-F"]
imrg_f <- imrg_f[, .(Ocean = ocn, Threshold = thresh, POD, FAR, CSI, H, M, FA, CN)]

imrg_f <- imrg_f[Threshold == 1 | Threshold == 25 | Threshold == 50 | Threshold == 99]


### export as a overleaf table

library(xtable)
print(xtable(imrg_f, type = 'latex'))
