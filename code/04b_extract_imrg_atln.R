...
atln_station <- atln_station[, .(lat, lon)] 
atln_station_df <- as.data.frame(atln_station)
...
data_list <- parLapply(cluster, file_name, extract_grids, atln_station_df) #19 min 48 sec 
parallel::stopCluster(cluster)
imrgf_dt <- rbindlist(data_list)

saveRDS(imrgf_dt, './data/atln_imrgf_extract.rds')
...
data_list <- parLapply(cluster, file_name, extract_grids, atln_station_df)
parallel::stopCluster(cluster)
imrgl_dt <- rbindlist(data_list)


saveRDS(imrgl_dt, './data/atln_imrgl_extract.rds')
...
file_name <- list.files("F:/IMERG/early/", pattern = ".nc4$", 
                        full.names = TRUE) %>% as.list()
...
saveRDS(imrge_dt, './data/atln_imrge_extract.rds')
