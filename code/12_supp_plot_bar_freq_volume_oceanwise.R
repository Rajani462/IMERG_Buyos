...
# calculate the frequencies and volume percetage of light (0.1 to 1 mm_ precip-----------------------------------

ind_ocn <- data_f[ocn == "Indian"]
ind_ocn[imrg_rf > 0 & imrg_rf < 1]

light_rain <- data_f[imrg_rf > 0 & imrg_rf < 1, .(N =.N, sum_rf = sum(imrg_rf)), by = .(ocn)]
tot_dat <- data_f[, .(tot_evnt = .N, tot_rf = sum(imrg_rf)), by = .(ocn)]

light_rain_tot <- light_rain[tot_dat, on = .(ocn)]

light_rain_tot[, `:=`(freq = (N/tot_evnt)*100, vol = (sum_rf/tot_rf)*100), by = .(ocn)]
light_rain_tot

# calculate the frequencies and volume percetage of light (>0 to 0.1_ precip-----------------------------------

ind_ocn <- data_f[ocn == "Indian"]
ind_ocn[imrg_rf > 0 & imrg_rf < 0.1]

light_rain <- data_f[imrg_rf > 0 & imrg_rf < 0.1, .(N =.N, sum_rf = sum(imrg_rf)), by = .(ocn)]
tot_dat <- data_f[, .(tot_evnt = .N, tot_rf = sum(imrg_rf)), by = .(ocn)]

light_rain_tot <- light_rain[tot_dat, on = .(ocn)]

light_rain_tot[, `:=`(freq = (N/tot_evnt)*100, vol = (sum_rf/tot_rf)*100), by = .(ocn)]
light_rain_tot[, lapply(.SD, round, 2), by = .(ocn, N)]
